namespace BreadLibrary.Common
{
    public abstract class BaseWhipProjectile : ModProjectile
    {
        ...
        public virtual SoundStyle WhipHitSFX => _WhipHitSFX;
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        protected virtual Rectangle HeadRect(int horizontalFrames = 1, int veritcalFrames = 1, int x = 1, int y = 1)
        {
        ...
        /// <summary>
        /// the amount of Segments used in the texture.
        /// where each segment is a rectangle.
        /// </summary>
        /// <remarks>Defaults to 5.</remarks>
        protected virtual int TextureSegmentCount => 5;

        public List<Rectangle> _TextureRectangles => new List<Rectangle>(TextureSegmentCount);
                ...
                switch (pass)
                {
                    ...
                    case WhipDrawPass.Head:
                        if (_ShouldDrawHead && WhipHead is not null)
                        {
                            ...
                            float apex = MathF.Sin(normalized * MathF.PI); // 0..1..0
                            Main.NewText(apex);
                            float apexAmplitude = _HeadScaleAmount;
                            float scaleAmount = 1f + apexAmplitude * apex * rangeMultiplier;
                            Vector2 Scale = new Vector2(scaleAmount);
                           //Main.NewText(scaleAmount);
        ...
        private void DrawWhipPrimitive(List<Vector2> points, Color BaseColor, float Alpha = 0, float baseWidth = 6f, float textureRepeats = 1f, float uOffset = 0f)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                Color lightingColor = Lighting.GetColor(p.ToTileCoordinates());
