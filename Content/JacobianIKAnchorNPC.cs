...
namespace BreadLibrary.Content
{
    ...
    public class JacobianIKAnchorNPC : ModNPC
    {
        ...
        alpha: 0.005f,
            ...
            particle.Prepare(Limb.JointPositions[^1]);
            ParticleEngine.ShaderParticles.Add(particle, PixelLayer.BehindTiles);

            /*
            MetaballSystem.Create<ToxicSludgeMetaball>(
            Limb.JointPositions[^1],
            radius: 90f,
            strength: 10f,
            opacity: 3f,
            timeLeft: 50,
            velocity: Main.rand.NextVector2Unit() * Main.rand.NextFloat(1, 4),
            initializer: (ref MetaballInstance instance) =>
            {
                instance.ai[0] = Main.rand.NextFloat(MathHelper.TwoPi);
                instance.ai[1] = Main.rand.NextFloat(0.8f, 1.2f);
            });
            */
        ...
        {
           // Main.EntitySpriteDraw(MetaballRenderer.postProcessTarget, NPC.Center - screenPos, null, Color.White, 0, Vector2.Zero, 1, 0);
